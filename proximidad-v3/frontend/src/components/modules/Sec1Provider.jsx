...
export default function Sec1Provider({ serviciosExitosos = 0, serviciosCreados = 0, calificacionGeneral = 0, proveedorId }) {
  ...
  // Estado para calificación promedio real
  ...
  // Cargar servicios del proveedor usando API proveedor
  const cargarMisServicios = async () => {
    if (!proveedorId) return
    
    setLoadingServicios(true)
    try {
      const response = await axios.get(`${config.API_BASE_URL}/proveedor/mis-servicios/?proveedor_id=${proveedorId}`)
      setMisServicios(response.data.servicios || [])
      console.log('✅ Servicios cargados desde API proveedor:', response.data.servicios)
    } catch (error) {
  ...
  // Cargar solicitudes recibidas usando API proveedor
  const cargarSolicitudesRecibidas = async () => {
    if (!proveedorId) return
    
    setLoadingSolicitudes(true)
    try {
      const response = await axios.get(`${config.API_BASE_URL}/proveedor/solicitudes/?proveedor_id=${proveedorId}`)
      setSolicitudesRecibidas(response.data.solicitudes || [])
      console.log('✅ Solicitudes recibidas desde API proveedor:', response.data.solicitudes)
    } catch (error) {
  ...
  const cargarCalificacionPromedio = async () => {
    ...
    try {
      const response = await axios.get(`${config.API_BASE_URL}/proveedor/calificacion/?proveedor_id=${proveedorId}`)
      setCalificacionReal(response.data.calificacion_promedio || 0)
      setTotalCalificaciones(response.data.total_calificaciones || 0)
      console.log('✅ Calificación promedio:', response.data.calificacion_promedio)
    } catch (error) {
  ...
  const handleResponderSolicitud = async () => {
    ...
    try {
      ...
      await axios.put(
        `${config.API_BASE_URL}/proveedor/solicitudes/${solicitudSeleccionada.id}/responder/`,
        ...
        <Row className="g-4">
          ...
          <Col lg={3} md={6}>
            <div className="metric-card" id="metric-rating">
              ...
              <div className="metric-content">
                <div className="stars-container d-flex gap-1 mb-2">{renderStars(calificacionReal)}</div>
                ...
                <p className="metric-description">{totalCalificaciones} {totalCalificaciones === 1 ? 'valoración' : 'valoraciones'}</p>
              </div>
            </div>
          </Col>
